...
@parser.route('/parse-and-save-csv')
def parse_and_save_csv():
    time_string = datetime.now().strftime('%Y_%m_%d_%H_%M_%S')

    columns = ['id', 'author_id', 'address', 'user_address', 'latitude', 'longitude', 'resale', 'floor', 'number_of_floors', 'number_of_rooms', 'area_total', 'area_living', 'area_kitchen', 'seller', 'created_at',
               'last_time_up', 'url',
               'photo', 'option1', 'option2', 'option3', 'option4', 'price_byn', 'price_usd']

    with open(csv_path + 'flats_minsk_{}.csv'.format(time_string), 'w', newline='') as file:
